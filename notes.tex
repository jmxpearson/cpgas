...
\begin{equation}
    \mathcal{C}_* = -\kappa - \log (1 + e^{-\kappa})
\end{equation}
This leads us to

\subsection*{Theorem:} The cost function
\begin{equation}
    \mathcal{C} = -\pi [N \log \nu + \lambda (\nu - 1)\ell] + \mathcal{H}(\pi)
\end{equation}
...
\end{equation}
In this equation, we consider $\lambda$, $\nu$, $N$, $\ell$, $N_i$, and $\ell_i$ as given, with $\pi$ chosen in each segment to maximize $\mathcal{C}$. We begin by noting that our division of the data will result in a $\kappa_i$ with each new segment satisfying $\kappa_1 + \kappa_2 = \kappa$. As a result, we rewrite $\kappa_1 \equiv \alpha \kappa$ and look for bounds of the form
\begin{equation}
